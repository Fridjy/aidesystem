...
The Frontend will be available on port 8989.

The Frontend communicates with The Backend in part using a session cookie. The cookie is `Secure`, but will be sent over HTTP on localhost. The Frontend also interacts with data collection clients and with services:
